...
중간 색인에 아직 충분한 근거가 없는 질문은 `coverage_gap`으로 표시합니다. 이 질문은 전체 데이터 색인 필요성을 확인하는 용도이며, core 검색 품질 점수에서는 분리해서 봅니다.
...
`legal_chunks_medium` 기준, 분야 필터와 키워드 보강을 함께 사용하면 32개 질문 중 30개 core 질문에서 분야 적중률과 키워드 적중률이 모두 100%입니다. 2개 행정법 질문은 중간 색인에 관련 근거가 거의 없어 `coverage_gap`으로 분리했습니다.
