...
DEFAULT_INPUT_PATH = Path("data/processed/precedents/precedent_documents.jsonl")
DEFAULT_OUTPUT_PATH = Path("data/chunks/precedents/precedent_chunks.jsonl")
        ...
        output.append(
            {
                ...
                "metadata": {
                    **metadata,
                    "case_number": document.get("case_number") or "",
                    "case_name": document.get("case_name") or "",
                    "court": document.get("court") or "",
                    "decision_date": document.get("decision_date") or "",
...
def write_chunks(
    ...
    if not input_path.exists():
        raise FileNotFoundError(f"Input precedent JSONL does not exist: {input_path}")
    if start_offset < 0:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Chunk normalized precedent documents for RAG retrieval.")
