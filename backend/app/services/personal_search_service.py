...
def search_personal_workspace(db: Session, user_id: int, query: str, limit: int = 40) -> list[dict[str, object]]:
    """Search one user's cases, notes, tasks, attachments, and conversations."""
    normalized_query = query.strip()
    if not normalized_query:
        return []
    pattern = f"%{_escape_like(normalized_query)}%"
    per_type_limit = max(5, min(20, limit))
    ...
    for note, case_title in note_rows:
        results.append(
